...
class GoogleApiFactory {
  ...
  Future<TasksApi?> createTasksApi() async {
    final client = await _manager
        .getAuthenticatedClient(GooglePermissionManager.tasksScopes);
  ...
  Future<CalendarApi?> createCalendarApi() async {
    final client = await _manager
        .getAuthenticatedClient(GooglePermissionManager.calendarScopes);
    if (client == null) return null;
  ...
  Future<DriveApi?> createDriveApi() async {
    final client = await _manager
        .getAuthenticatedClient(GooglePermissionManager.driveScopes);
    if (client == null) return null;
