...
Retained: raw source file, extraction output, normalized plan, every resolution decision, and the approved revision. **Every retained artifact is subject to the §1 medical-content screen — not just the raw file.** Because the classifier reads extracted/OCR text, that derived text can itself carry flagged content, so any flagged span is redacted from (or withheld across) the raw file *and* every derived text artifact (extraction output, normalized plan) before persistence to this repository; a file that cannot be fully inspected is withheld entirely. The §1 fail-closed boundary governs; §6 never overrides it.
...
**Later review passes (2026-07) — deferred to the implementation PR as tracked items (in-body spec expansion stopped here):**

- **Lease fencing for uncertain creates** (P1). A create that outlives its lease can duplicate: the recovery runner reconciles before the first object is visible, sees no marker, re-leases, and creates again while the first later commits. An expired slot must stay in reconciliation with bounded polling (or a fencing token / quiescence window) until the earlier request provably cannot still commit, before another create is permitted.
- **Credential-swap ↔ write TOCTOU** (P1). `HevyIntegrationController.connect()` saves a new API key before verification and `HevyApiClient` reads the credential per request, so an account-A command can pass its account check and then be sent with account B's key before the new `hevyAccountId` is retained. Credential changes and outbound writes must share a lock, or every request must bind to an immutable verified key/account session.
- **⚠️ Remote-edit conflict detection before supersession** (P1, *safety-relevant*). If the user edits routine *N* directly in Hevy, in-place update or delete-after-create silently destroys those changes. Store a remote version or canonical content hash and route a mismatch through conflict review before updating/deleting.
- **⚠️ Constraint-bearing medical redaction** (P1, *safety-relevant*). Redacting a medical span that is *also* an operative training constraint (e.g. "avoid overhead work", a pain-limited range) can approve a materially different, unsafe routine. Treat constraint-bearing redactions as unresolved and block publication unless the plan is safely re-specified or handled through a protected path.
- **Catalog snapshot in resolution provenance** (P2). A refreshed exercise-template catalog can change which template a name resolves to, but provenance records only `resolutionDecisionVersion`. Persist a catalog snapshot hash / as-of identifier and the candidate set with each resolution decision so the choice is reconstructible.
- **Removal intents for deleted sessions/weeks** (P1). A corrected plan that drops a session or an entire week has no revision *N+1* for that slot, so the supersession path (§7) never runs and revision *N*'s routine stays live in Hevy indefinitely. Reconcile the full old/new session sets and enqueue idempotent removal intents for deleted slots, not just replacements.
- **XLSX cached-formula values** (P1). Extractors often expose a workbook's *cached* formula result, which is stale if the file was saved without recalculation; that value then passes normalization and domain validation while differing from the visible formula. Preserve formula metadata and either evaluate it with defined semantics or route formula-derived prescriptions through review rather than trusting cached cells as deterministic raw input.
- **Per-set rest is not Hevy-writable** (P1, *corrects §2*). §2 lists per-set rest as a normalized field, but Hevy routine payloads expose `rest_seconds` at the **exercise** level only — `HevySet` has no rest field. Restrict rest to exercise-level values, or route non-uniform per-set rest through review; the §2 "rest timer — per-set or per-exercise" wording must be corrected in implementation so nothing is silently dropped or mis-mapped.
- **⚠️ On-device OCR for pre-classification** (P1, *privacy/safety*). The §1 attestation requires OCR of image-bearing pages but does not say *where* it runs; uploading an unscreened, possibly medical-tier page to a third-party cloud OCR would breach the boundary before the classifier acts. Require on-device/offline OCR (or an explicitly approved protected-processing boundary) for pre-classification content.
- **Override targets revalidated against the current catalog** (P2). A saved override can point to a custom Hevy template that was later deleted; auto-resolving it (§2 order) yields an approved command targeting a nonexistent template that cannot publish. Require every alias/override target to exist in the current account catalog and send missing targets to exercise lookup/review.
- **Drift migration test for the new tables** (P1, *DoD gap*). Persisting drafts, overrides, decisions, plans, commands, and the schema-level provenance constraints bumps the Drift schema. `CLAUDE.md` requires a migration test for **every** schema bump, but §10 does not list one. The implementation's DoD must add an old-schema→new-schema migration test covering every PlanIngestion table and index (clean-DB tests alone would let existing v7 installs upgrade without the new tables and fail at runtime).

> **Status note:** the header remains "Approved architecture — BLOCKED on PR E." The first three review passes' P1s are resolved in-body; later passes are recorded above as tracked items for the implementation PR (two flagged ⚠️ safety-relevant — remote-edit conflict detection and constraint-bearing redaction — for the owner's attention). In-body expansion of this architecture doc was intentionally stopped to avoid unbounded growth; these items are design work for implementation, not blockers on the architecture itself.
